...
#     rmask = cv.inRange(hsv, lower_red, upper_red)

#     # Bitwise-AND mask and original image
#     rres = cv.bitwise_and(frame,frame, mask= rmask)
#     bres = cv.bitwise_and(frame, frame, mask = bmask)
#     tres = cv.bitwise_or(rres, bres)
#     masks = [gmask, bmask, rmask]
#     # add masks
#     h, w, c = frame.shape
    
    
#     cv.imshow('frame',frame)
#     cv.imshow('redResult', rres)
#     cv.imshow("blueResult", bres)
#     cv.imshow("totalResult", tres)
#     cv.imshow('blue mask',bmask)
#     cv.imshow('green mask',gmask)
#     cv.imshow('red mask', rmask)
